...
namespace OpenBots.Commands.Loop
{
	...
	public class BeginForEachCommand : ScriptCommand
	{
		[Required]
		...
		{
			BeginForEachCommand loopCommand = (BeginForEachCommand)parentCommand.ScriptCommand;
            ...
            try
            {
				loopTimes = dynamicLoopVariable.Count;
			}
			catch (Exception ex)
            {
				try
				{
					loopTimes = dynamicLoopVariable.Length;
				}
				catch (Exception)
				{
					throw ex;
		...
		{
			return $"For Each '{v_OutputUserVariableName}' in '{v_LoopParameter}'";
		}
	}
}