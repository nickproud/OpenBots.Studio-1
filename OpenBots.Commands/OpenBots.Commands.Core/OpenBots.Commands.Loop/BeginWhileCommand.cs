...
namespace OpenBots.Commands.Loop
{
	...
	public class BeginWhileCommand : ScriptCommand, IConditionCommand
	{
		[Required]
		...
		public BeginWhileCommand()
		{
			CommandName = "BeginWhileCommand";
			SelectionName = "While";
			CommandEnabled = true;
		...
		{
			if (v_Option == "Inline")
				return $"While ({v_Condition})";
			else
				return _commandHelper.GetConditionDisplayValue(v_ActionType, v_ActionParameterTable, "While");
		}
