...
namespace OpenBots.Core.Utilities.CommonUtilities
{
    public static class StringMethods
    {
        ...
        {
            if (!t.IsGenericType && t.BaseType?.FullName != "System.Array")
                return t.FullName;
            ...
            if (index != -1)
            {
                stringBuilder.Append(t.FullName.Substring(0, index));
                ...
                if (t.BaseType?.FullName == "System.Array")
                    stringBuilder.Append("[]");
        ...
        public static string GetRealTypeName(this Type t)
        {
            ...
            if (index != -1)
            {
                stringBuilder.Append(t.Name.Substring(0, index));
                stringBuilder.Append('<');
                bool appendComma = false;
                foreach (Type arg in t.GetGenericArguments())
                {
